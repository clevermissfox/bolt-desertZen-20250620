...
export default function AuthBridgeScreen() {
  ...
  useEffect(() => {
    ...
    const processCallback = async () => {
      try {
        console.log('🔄 Processing auth bridge callback...');
        console.log('📋 Bridge params:', JSON.stringify(localSearchParams, null, 2));

        // Set a maximum timeout to prevent infinite loading
        timeoutId = setTimeout(() => {
          console.log('⏰ Bridge processing timeout, redirecting to auth');
        ...
        // Also check the current URL if we're on web
        ...
        // Handle errors first
        if (error) {
          console.error('❌ Auth bridge error:', error, error_description);
        ...
        setTimeout(() => {
          router.replace({
            pathname: '/auth',
            params: { error: 'Bridge processing failed' },
          });
        }, 2000);
      } finally {
        setProcessing(false);
